class AI5 extends AI4
  ...
  search_not_final: ->
    ...
    for i, [row, col] of @board.movable_cells()
      ...
      temp_min_score = @search_second_move(next_board, 0, max_score)
  ...
  search_second_move:(board, depth, max_score) ->
    if @not_final_depth_limit <= depth
      board.change()
      return @evaluate_not_final(board)
  ...
  search_not_final_best_of_AI:(board, depth, min_score) ->
    return @evaluate_not_final(board) if @not_final_depth_limit <= depth
    movable_cells = board.movable_cells()
    max_score = -64
    if movable_cells.length is 0
      board.change()
